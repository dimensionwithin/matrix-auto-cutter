...
Der geprüfte Projekt-Root ist `P:\DimensionWithin-MatrixMarketAutoEditor`. Dort liegen:
...
- Die in v0.4 genannten Workflow-Vorgaben `F:\VIDEO ROHABLAGE` für Quellen und `F:\MatrixMarketAutoEdit` für Exporte bleiben konfigurierbare Standardpfade. Der tatsächliche Code-/Planungs-Root ist der in Abschnitt 2 belegte Root.
...
- der in v0.4 genannte Repo-Pfad `P:\DimensionWithin\Matrix Auto Cutter` wird durch den tatsächlich geprüften Root `P:\DimensionWithin-MatrixMarketAutoEditor` ersetzt.
